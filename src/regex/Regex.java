...
class Regex {
    public static void main(String[] args) {
        ...
        System.out.println("The following pattern ('\\d'){36} matches : all the digits with 36");
        ...
        for(int i = 0; i < TEKmentors.length; i++) {
            ...
            tekMentors[i] = temp.replaceAll("(\\w+)\\s\\w+", "$1");
        }
        ...
        String replace = sleepy.replaceAll("sleepy", "");
        String[] replaceSleep = replace.split("\\s+");
        System.out.println(Arrays.toString(replaceSleep));

        // 4.b combine the array that you just created into a string
       
        String not_sleepy; //punctuation marks will be here
        not_sleepy = String.join(" ", replaceSleep);
